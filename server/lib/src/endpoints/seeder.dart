import 'package:bita_pod_server/src/generated/catagory.dart';
import 'package:serverpod/serverpod.dart';

insertData(Session session) async {
  final data = {
  ...
  for (var entry in data.entries) {
    final entity =
        await Catagory.db.insertRow(session, Catagory(name: entry.key));

    final entries = <Catagory>[];

    for (var entry2 in entry.value) {
      entries.add(Catagory(name: entry2, catagoryId: entity.id));
    }
    await Catagory.db.insert(session, entries);
  }
}
